...
public class HomeControllersTests
{
    ...
    public async void Post_GetPostById_ReturnsViewWithCorrectPost()
    {
        ...
        postReadRepo.Setup(p => p.RetrieveByIdAsync(It.IsAny<long>(), It.IsAny<bool>())).ReturnsAsync(post);
    ...
    {
        postReadRepo.Setup(p => p.RetrieveByIdAsync(It.IsAny<long>(), It.IsAny<bool>())).ReturnsAsync((Post?)null).Verifiable();
        ...
        postReadRepo.Verify(p => p.RetrieveByIdAsync(1, It.IsAny<bool>()), Times.Once());
