...
public class UserSignUpValidator : AbstractValidator<UserSignUpDTO>
{
    public UserSignUpValidator(IUserService userService)
    {
        ...
        RuleFor(u => u.Username).NotEmpty().MustAsync(async (u, token) => (u == null || (await userService.GetUserByNameAsync(u)) == null)).WithMessage("User {PropertyValue} already exists.");
