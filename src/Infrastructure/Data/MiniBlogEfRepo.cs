...
public partial class MiniBlogEfRepo : IMiniBlogRepo
{
    ...
    public async Task<long?> CreatePost(Post post)
    {
        ...
        if (changes > 0) return post.Id;
        return null;
    }

    public async Task<Post?> RetrievePost(long Id, PaginateParams commentsParams)
    {
        Post? post = await context.Posts.Include(p => p.Tags).Where(p => p.Id == Id).FirstOrDefaultAsync();
    ...
    {
        Post? existing = await context.Posts.Include(t => t.Tags).FirstAsync(p => p.Id == post.Id);
    ...
    {
        Commentary? comment = await context.Commentaries.FirstOrDefaultAsync(c => c.Id == commId);
        if (comment != null)
        {
